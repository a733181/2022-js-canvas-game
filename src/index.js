const illustrateBtnEl = document.querySelector('.illustrate-btn');
const illustrateEl = document.querySelector('.illustrate-container');

const selectEl = document.querySelector('.select-user');
const selectFormEl = document.querySelector('.select-form');
...
const endImage = document.querySelector('.end-img');

const restartBtnEl = document.querySelector('.restart');
const backSelectBtnEl = document.querySelector('.back-select');
...
illustrateBtnEl.addEventListener('click', () => {
  illustrateEl.classList.add('none');
});

selectFormEl.addEventListener('submit', (e) => {
  e.preventDefault();
...
backSelectBtnEl.addEventListener('click', () => {
  resetGameHandler();
  selectEl.classList.remove('none');
  canvasEl.classList.add('none');
});
...
function endGameHandler() {
  ...
  endEl.classList.remove('none');
  if (lifes.length === 0) {
    endTitleEl.innerText = 'OPS !';
  } else {
    endTitleEl.innerText = 'YOU WIN !';
    endImage.classList.remove('none');
    endImage.src = `./src/img/role/${playerImages[playerIndex].name}.png`;
  }
}

function resetGameHandler() {
  endEl.classList.add('none');
  obstacles = [];
  obstacleTimer = 0;
  lastTime = 0;
